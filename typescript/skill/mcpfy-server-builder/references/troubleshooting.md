...
- [ ] If `auth` is set, `type` is `"oauth"` or `"header"` — never `"jwt"`. If `"oauth"`, `authorizationServers` is present (not required for `"header"`).
...
| `auth: { type: "jwt", ... }` rejected | Use `type: "oauth"` (full JWT/JWKS verification) or `type: "header"` (simple bearer check) — there's no `"jwt"` type |
| "authorizationServers is missing" | Forgot `authorizationServers: [issuer]` alongside `auth` |
