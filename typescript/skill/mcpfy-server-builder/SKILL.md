...
3. **Read only the reference files the task needs:**
   ...
   - [Authentication](references/auth.md) — `type: "oauth"` (`jwksVerifier`, Auth0/WorkOS presets) or `type: "header"` protection for an HTTP server, and client-side OAuth (`mcpfy-sdk/auth`'s `NodeOAuthClientProvider`, `ensureAuthorized`).
...
- Auth configuration is `type: "oauth"` (full OAuth/JWT verification) or `type: "header"` (a simple bearer-token check) — there's no `"jwt"` discriminant. `authorizationServers` is required for `type: "oauth"` specifically, not for `type: "header"`.
